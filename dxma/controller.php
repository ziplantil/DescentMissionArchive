...
class DatabaseController {
    ...
    public function editUser(int $uid, array $data, $auth) {
        ...
        if (!$this->db->execute("UPDATE User SET realname=?, email=?, website=?, description=?, forgotcode=NULL WHERE id=?",
        $realname, $email, $website, $desc, $uid)) {
            return fail("Could not update user");
        }
        return TRUE;
    }
    ...
    public function setPassword(int $uid, array $data, $auth) {
        ...
        if (!$this->db->execute("UPDATE User SET passhash=?, forgotcode=NULL WHERE id=?",
        $phash, $uid))
    ...
    public function setUpForgot(int $uid, string $ticket) {
        return $this->db->execute("UPDATE User SET forgotcode=?, forgotexpiry=DATE_ADD(NOW(), INTERVAL 24 HOUR) WHERE id=?",
            $ticket, $uid);
    }
    ...
    public function createMission(int $uid, string $uname, array $data) {
        ...
        if (empty($title)) return $this->fail("Empty title not permitted");
        $version = $data["version"];
        if (empty($version)) return $this->fail("Empty version not permitted");
        $author = $data["author"];
        if (empty($author)) $author = $uname;
        $desc = $data["description"];
        if (empty($desc)) $desc = "";
        if (strlen($desc) > DESC_MAXLENGTH) return fail("Description is too long");
        $mode = $data["mode"];
        if (!is_numeric($mode)) return $this->fail("Invalid mode");
        $mode = intval($mode);
        if (!isset(MODE_ENUM[$mode])) return $this->fail("Invalid mode");
        $game = $data["game"];
        if (!is_numeric($game)) return $this->fail("Invalid game");
        $game = intval($game);
        if (!isset(GAME_ENUM[$game])) return $this->fail("Invalid game");
        $levels = $data["levels"];
        if (!is_numeric($levels)) return $this->fail("Invalid level count");
        $levels = intval($levels);
        if ($levels < 1 || $levels > 255) return $this->fail("Invalid level count");
        $playersMin = $data["playersMin"];
        if (!is_numeric($playersMin)) return fail("Invalid player count");
        $playersMin = intval($playersMin);
        if ($playersMin < 1 || $playersMin > 255) return fail("Invalid player count");
        $playersMax = $data["playersMax"];
        if (!is_numeric($playersMax)) return fail("Invalid player count");
        $playersMax = intval($playersMax);
        if ($playersMax < 1 || $playersMax > 255) return fail("Invalid player count");
        if ($playersMin > $playersMax) swap($playersMin, $playersMax);
        $released = $data["released"];
        if (empty($released) || !validateDate($released)) $released = date("Y-m-d");
        ...
        $ok = $this->db->execute("INSERT INTO Mission (title, version, author, user, description, mode, game, levels, playersMin, playersMax, released, filename, screenshot, created, updated) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, now(), now())",
            $title, $version, $author, $uid, $desc, $mode, $game, $levels, $playersMin, $playersMax, $released, $fname, $sname);
        if (!$ok) {
            deltree(getMissionFilePath($uid, $tempdir));
    ...
    public function editMission(int $uid, int $mid, string $uname, array $data) {
        $title = trim($data["title"]);
        if (empty($title)) return fail("Empty title not permitted");
        $version = $data["version"];
        if (empty($version)) return fail("Empty version not permitted");
        $author = $data["author"];
        if (empty($author)) $author = $uname;
        $desc = $data["description"];
        if (empty($desc)) $desc = "";
        if (strlen($desc) > DESC_MAXLENGTH) return fail("Description is too long");
        $mode = $data["mode"];
        if (!is_numeric($mode)) return fail("Invalid mode");
        $mode = intval($mode);
        if (!isset(MODE_ENUM[$mode])) return fail("Invalid mode");
        $game = $data["game"];
        if (!is_numeric($game)) return fail("Invalid game");
        $game = intval($game);
        if (!isset(GAME_ENUM[$game])) return fail("Invalid game");
        $levels = $data["levels"];
        if (!is_numeric($levels)) return fail("Invalid level count");
        $levels = intval($levels);
        if ($levels < 1 || $levels > 255) return fail("Invalid level count");
        $playersMin = $data["playersMin"];
        if (!is_numeric($playersMin)) return fail("Invalid player count");
        $playersMin = intval($playersMin);
        if ($playersMin < 1 || $playersMin > 255) return fail("Invalid player count");
        ...
        if (!$this->db->execute("UPDATE Mission SET title=?, version=?, author=?, description=?, mode=?, game=?, levels=?, playersMin=?, playersMax=?, released=? WHERE id=?",
        $title, $version, $author, $desc, $mode, $game, $levels, $playersMin, $playersMax, $released, $mid)) {
    ...
    public function updateMissionScreenshot(int $uid, int $mid, array $data) {
        ...
        $fname = $data['name'];
        if (empty($fname)) $fname = "image.png";
        $fext = strtolower(getExtension($fname));
